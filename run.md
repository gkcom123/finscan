...
**Written:** Model!AH (11v/27f)  
**File:** `inbox/Tencent_test_updated_2.xlsx`
...
| Model!13 | Other gains, net | other_income | 2,820.00 | (derived) | alias | 0.85 |
...
- **composite_incomplete** (other_income): other_income composite expects ['other_income', 'investment_gains'] but the filing only yielded ['investment_gains']. The figure may be short.
- **ambiguous_row** (ebitda): Model: rows 40 and 37 both resolve to 'ebitda' ('EBITDA' / 'EBITDA'). Row 37 kept; row 40 left alone.
- **unmapped_extraction**: Extracted from the PDF but no matching row in the workbook: Earnings per share - Basic, Earnings per share - Diluted, Net gains/(losses) from investments and others, Other comprehensive income, Total comprehensive income, Total income
...
- **profile_reused**: Reusing the layout profile for 'tencent' (fingerprint 361d6a0362d19961, confirmed).
- **profile_fingerprint_match**: No reliable company name in the PDF; matched the existing 'tencent' profile by workbook layout (fingerprint 361d6a0362d19961).
...
- **composite_applied** (other_income): Other income written as 2,820,000,000 = Net gains/(losses) from investments and others 2,820,000,000, per this company's saved profile.
...
- **rows_unmatched**: 24 workbook row(s) left blank (headings/unknown captions): r7:Model!% of growth, r10:Model!Gross Profit margin, r11:Model!Operating expenses:, r16:Model!Operating profit, r22:Model!% margin, r26:Model!% of sales or stated otherwise, r27:Model!=B12, r28:Model!=B13 ...
